...
namespace Stumblr
{
	...
	///
	/// It is worth reporting anyway, because UL is where the interesting failures would be. UL
	/// replaces EntityMoveHelper.UpdateMoveHelper wholesale with a prefix that never returns true,
	/// which is why the zombie hook hangs off StartJump instead - see Patches. And UL is where the
	/// Athletics perk comes from: without it, TripChance falls back to vanilla's perkParkour. If
	/// either ever stops holding, the first thing anyone will want to know is which build they were
	/// running.
	///
	...
	internal static class UndeadLegacyInfo
	{
		...
		/// <summary>One-line summary for the <c>fw</c> console command.</summary>
		...
		internal static void Report()
		{
			...
			if (!Present)
			{
				...
				Log.Out(Patches.LogPrefix + "Undead Legacy is not installed; running against vanilla "
					+ "jumping. Trips still work - only the Athletics scaling changes, falling back "
					+ "to vanilla's perkParkour.");
				return;
			}
